...
regl.frame(function () {
  regl.clear({ color: [0,0,0,0.1], depth: true })
  camera(function () {
    draw.blob()
    draw.floor()
  })
...
function createFloor (regl) {
  ...
  return regl({
    frag: glsl`
      ...
      varying vec3 vpos;
      void main () {
        float h = (snoise(vec4(vpos*0.3,time*0.2))*0.5+0.5)*0.3+0.9;
        float s = (snoise(vec4(vpos*0.1,time*0.1))*0.5+0.5)*0.4+0.9;
        float l = (pow(snoise(vec4(vpos,time*1.0))*0.5+0.5,8.0)+0.9)
          * sin(vpos.x);
...
function createBlob (regl) {
  ...
  return regl({
    frag: glsl`
      ...
      uniform float time;
      void main () {
        float h = (snoise(vec4(vpos,time))*0.3+0.5)*0.3
          + mod(time*0.2,1.0);
        float l = pow(snoise(vec4(vpos,time*0.2))*0.5+0.5,2.0);
        vec3 color = hsl2rgb(h,0.1,l);
    ...
    vert: glsl`
      ...
      void main () {
        vpos = position - normal
          * snoise(vec4(position,time*0.2))*0.01;
        gl_Position = projection * view * vec4(vpos,1);
      }
    `,
    attributes: {
